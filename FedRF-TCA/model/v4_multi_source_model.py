...
class federated_source_and_target_model(nn.Module):
    def __init__(self, feature_dim, m, n_features, sigma, gamma, kernel='rbf', class_num=31):
        ...
        (3) In the end, all Source parties' classifier vote for the result of Target classification. 
        """
        super(federated_source_and_target_model, self).__init__()
        # feature extractor
        ...
        self.source4_rf_map = federated_rf_map(n_features=n_features, sigma=sigma, feature_dim=feature_dim, kernel=kernel, domain='source')
        ...
        self.source4_rf_map.update_rf_map(self.source1_rf_map.get_rf_map())
        ...
        self.Wrf_source4 = nn.Linear(2*n_features, m, bias=False)
        ...
        self.source4_classifier = bottleneck(input_features_dim=m, output=class_num).requires_grad_(True)
    ...
    def forward(self, x_s1, x_s2, x_s3, x_s4, x_t):
        ...
        source4_Sigma_i, source4_Sigma_yi = self.source4_rf_map(x_s4)
        ...
        fs4 = self.Wrf_source4(source4_Sigma_i.T)
        fs4 = self.sample_norm(fs4)
        ys4 = self.source3_classifier(fs4)
        ...
        return source1_Sigma_yi, source2_Sigma_yi, source3_Sigma_yi, source4_Sigma_yi, target_Sigma_yi, ys1, ys2, ys3, ys4, yt # fs1, fs2, fs3, ft, # avoid user data leakage
    ...
    def multi_prediction_for_target(self, ft):
        ...
        yt4 = self.source4_classifier(ft)
        return torch.cat([yt1.unsqueeze(1), yt2.unsqueeze(1), yt3.unsqueeze(1), yt4.unsqueeze(1)], dim=1)
    ...
    def FedAvg_transfer_layer(self, subset_list=None):
        ...
        if subset_list == None:
            ...
            self.Wrf_source4.load_state_dict(fed_state_dict)
        else:
            if 0 in subset_list:
                self.Wrf_source1.load_state_dict(fed_state_dict)
            if 1 in subset_list:
    ...
    def FedAvg_only_target_classifier(self, cls_acc1, cls_acc2, cls_acc3, cls_acc4):
        models_num = 4 # source and target, 2 parties
        ...
        model_weights = [cls_acc1, cls_acc2, cls_acc3, cls_acc4]
        ...
        # update fed weights to fed model
        self.target_classifier.load_state_dict(fed_state_dict)
        return model_constant, (model_weights[0] / model_constant), (model_weights[1] / model_constant), (model_weights[2] / model_constant), (model_weights[3] / model_constant)
    ...
    def FedAvg_pair_transfer_layer(self, iter_num):
        ...
        if iter_num == 0:
            ...
            models_list = [self.Wrf_source4, self.Wrf_target]
        models_state_dict = [x.state_dict() for x in models_list]
        weight_keys = list(models_state_dict[0].keys())
        fed_state_dict = collections.OrderedDict()
        for key in weight_keys:
