...
if __name__ == "__main__":
    ...
    mnist_dir = os.path.abspath("./data/MNIST")
    mnistm_dir = os.path.abspath("./data/MNIST_M")


    data_train = datasets.MNIST(root = mnist_dir,
                                train = True,
                                download = False)
    X_train = []
    y_train = []
    for data in data_train:
        X_train.append(np.array(data[0]))
    ...
    data_test = datasets.MNIST(root=mnist_dir,
                            train = False)
    X_test = []
    y_test = []
    for data in data_test:
        print(data)
        X_test.append(np.array(data[0]))
    ...
    # 加载BSDS500数据集
    BST_PATH = os.path.abspath('./data/BSR_bsds500.tgz')
